...
app.post(`${PREFIX}/documents`, async (c) => {
  ...
  try {
    ...
    const name = String(formData.get('name') || file?.name || 'مستند').trim();
    if (!file) return c.json({ error: 'No file provided' }, 400);
    const MAX_DOCUMENT_BYTES = 20 * 1024 * 1024;
    const allowedExtensions = new Set(['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'jpg', 'jpeg', 'png', 'webp', 'gif', 'heic', 'heif']);
    const fileExt = (file.name.split('.').pop() || '').toLowerCase();
    if (!allowedExtensions.has(fileExt)) return c.json({ error: 'Unsupported file type' }, 415);
    if (file.size > MAX_DOCUMENT_BYTES) return c.json({ error: 'File exceeds 20 MB limit' }, 413);
    if (!name || name.length > 160) return c.json({ error: 'Invalid document name' }, 400);
