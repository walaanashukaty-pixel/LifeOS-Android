...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let providerKey = Deno.env.get('OPENAI_API_KEY') || '';
    if (!providerKey) {
      try {
        const { data: vaultKey, error: vaultError } = await supabase.rpc('lifeos_get_openai_api_key');
        if (vaultError) console.error('OpenAI Vault lookup failed', vaultError);
        if (typeof vaultKey === 'string') providerKey = vaultKey;
      } catch (error) {
        console.error('OpenAI Vault lookup failed', error);
      }
    }
    const providerModel = Deno.env.get('LIFEOS_AI_MODEL') || 'gpt-5.6-terra';
    if (!providerKey) {
      return json({
        code: 'OPENAI_API_KEY_MISSING',
        error: 'OpenAI API key is not configured on the LifeOS server.',
        conversationId,
      }, 503);
    }
