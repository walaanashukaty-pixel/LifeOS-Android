...
test('OpenAI Responses API request uses input blocks and json_object output format', async () => {
  let requestBody;
  const fetchImpl = async (_url, init) => {
    requestBody = JSON.parse(init.body);
    return {
  ...
  const result = await callLifeOSProvider({
    endpoint: 'https://api.openai.com/v1/responses', apiKey: 'secret', model: 'gpt-5.6-terra',
    message: 'مرحبا', context: {}, mode: 'chat', fetchImpl,
  });
