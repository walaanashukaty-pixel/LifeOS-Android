            ...
            <div key={doc.id} className="bg-card rounded-xl border border-border hover:border-primary/30 transition-all group">
              ...
              <div className="p-3">
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button onClick={() => { setEditDoc(doc); setEditDocForm({ name: doc.name || '', category: doc.category || 'عام' }); }} className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" aria-label="تعديل الوثيقة">
                    <Edit3 size={13} />
                  </button>
                  <button onClick={() => deleteDocument(doc.id)} className="p-2 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors" aria-label="حذف الوثيقة">
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
