...
import { Plus, Trash2, Search, Languages, Loader2, Edit3 } from 'lucide-react';
...
export function LanguagesPage() {
  ...
  async function saveLang() {
    const validationError = textValidation(langForm.name, 'اسم اللغة', 80);
    if (validationError) { toast.error(validationError); return; }
    try {
      if (editLang) {
        const updated = await api(`/languages/${editLang.id}`, { method: 'PUT', body: JSON.stringify({ ...editLang, ...langForm }) });
  ...
  async function saveItem() {
    ...
    if (tab === 'vocab') {
      ...
      newItem = { id: crypto.randomUUID(), word: itemForm.word, meaning: itemForm.meaning, example: itemForm.example, createdAt: new Date().toISOString() };
    } else if (tab === 'grammar') {
      if (!itemForm.title.trim()) { toast.error('أدخل عنوان القاعدة'); return; }
      newItem = { id: crypto.randomUUID(), title: itemForm.title, explanation: itemForm.explanation, notes: itemForm.notes, createdAt: new Date().toISOString() };
    } else {
      if (!itemForm.phrase.trim()) { toast.error('أدخل العبارة'); return; }
      newItem = { id: crypto.randomUUID(), phrase: itemForm.phrase, notes: itemForm.notes, createdAt: new Date().toISOString() };
    }
    try {
      ...
      if (!editItem && !await guardCreation({ key: 'language_content', currentCount: allContent.length })) return;
      const nextItems = editItem
        ? items.map((item: any) => item.id === editItem.id ? { ...newItem, id: editItem.id, createdAt: editItem.createdAt || newItem.createdAt } : item)
        : [...items, newItem];
      const updated = await api(`/languages/${selected.id}`, { method: 'PUT', body: JSON.stringify({ ...selected, [tab]: nextItems }) });
      setSelected(updated);
      setLanguages(l => l.map(x => x.id === updated.id ? updated : x));
      setShowItemForm(false);
      setEditItem(null);
      setItemForm({ word: '', meaning: '', example: '', rule: '', title: '', explanation: '', phrase: '', notes: '' });
      toast.success(editItem ? 'تم التعديل' : 'تمت الإضافة');
    } catch { toast.error('فشل الإضافة'); }
  }

  async function deleteItem(itemId: string) {
    if (!selected) return;
    if (!(await confirmAction({ title: 'حذف العنصر', description: 'سيتم حذف هذا العنصر من محتوى اللغة الحالية.' }))) return;
  ...
  async function deleteLang(id: string) {
    if (!(await confirmAction({ title: 'حذف اللغة', description: 'سيتم حذف اللغة وكل المفردات والقواعد والمحادثات المسجلة داخلها.' }))) return;
    try {
  ...
  const totalVocab = languages.reduce((sum, lang) => sum + (lang.vocab || []).length, 0);
  const totalGrammar = languages.reduce((sum, lang) => sum + (lang.grammar || []).length, 0);
  const totalConversation = languages.reduce((sum, lang) => sum + (lang.conversation || []).length, 0);

  if (loading) return <LoadingPage label="جارٍ تجهيز مساحة اللغات" />;

  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <PageHero eyebrow="LANGUAGE LAB" title="تعلم اللغات" description="اجمع المفردات والقواعد والمحادثات لكل لغة، وشاهد حجم مكتبتك اللغوية بسرعة." icon={<Languages size={20} />} actions={<button onClick={() => { setEditLang(null); setLangForm({ name: '', flag: '🌍', level: 'A1' }); setShowLangForm(true); }} className="lifeos-primary-action"><Plus size={16} /> لغة جديدة</button>} meta={<div className="lifeos-v3-hero-meta"><span className="lifeos-chip">{languages.length} لغة</span><span className="lifeos-chip">{totalVocab} مفردة</span><span className="lifeos-chip">{totalGrammar + totalConversation} مادة تطبيقية</span></div>} />

      <div className="lifeos-v3-summary-grid">
        <SummaryCard value={languages.length} label="اللغات النشطة" />
        <SummaryCard value={totalVocab} label="المفردات المحفوظة" />
        <SummaryCard value={totalGrammar} label="قواعد" />
        <SummaryCard value={totalConversation} label="محادثات وعبارات" />
      </div>

      <RewardCapacityBar rewardKey="languages" currentCount={languages.length} />

      {/* New Language Form */}
      {showLangForm && (
        <FormModal open={showLangForm} title={editLang ? 'تعديل اللغة' : 'إضافة لغة جديدة'} onClose={() => { setShowLangForm(false); setEditLang(null); }}>
        <div className="bg-card rounded-2xl border border-primary/30 p-5 space-y-3">
          <h3 className="font-semibold text-foreground">{editLang ? 'تعديل اللغة' : 'إضافة لغة جديدة'}</h3>
          ...
          <div className="flex gap-3 justify-end">
            <button onClick={() => { setShowLangForm(false); setEditLang(null); }} className="px-4 py-2 rounded-xl text-sm border border-border text-muted-foreground">إلغاء</button>
            <button onClick={saveLang} className="px-4 py-2 rounded-xl text-sm bg-primary text-primary-foreground">{editLang ? 'حفظ التعديل' : 'إضافة'}</button>
          </div>
        </div>
        </FormModal>
      )}
      ...
      {languages.length === 0 ? (
        <AnimatedEmptyState icon={Languages} title="ابدأ لغة جديدة" description="أضف اللغة التي تتعلمها ثم اجمع المفردات والقواعد والمحادثات في مكان واحد." action={<button onClick={() => { setEditLang(null); setLangForm({ name: '', flag: '🌍', level: 'A1' }); setShowLangForm(true); }} className="lifeos-primary-action"><Plus size={15} /> إضافة لغة</button>} />
      ) : (
        <div className="lifeos-v3-learning-layout">
          ...
          <div className="lifeos-v3-side-rail">
            {languages.map(lang => (
              <button
                ...
                data-active={selected?.id === lang.id}
          ...
          {selected && (
            <div className="space-y-4">
              ...
              <div className="bg-card rounded-xl border border-border p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => { setEditLang(selected); setLangForm({ name: selected.name || '', flag: selected.flag || '🌍', level: selected.level || 'A1' }); setShowLangForm(true); }} className="p-2 hover:bg-muted rounded-lg text-muted-foreground" aria-label="تعديل اللغة">
                      <Edit3 size={14} />
                    </button>
                    <button onClick={() => deleteLang(selected.id)} className="p-2 hover:bg-destructive/10 rounded-lg text-muted-foreground hover:text-destructive" aria-label="حذف اللغة">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
              ...
              <RewardCapacityBar
                rewardKey="language_content"
                currentCount={languages.reduce((total, lang) => total + (lang.vocab || []).length + (lang.grammar || []).length + (lang.conversation || []).length, 0)}
              />
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => { setEditItem(null); setItemForm({ word: '', meaning: '', example: '', rule: '', title: '', explanation: '', phrase: '', notes: '' }); setShowItemForm(true); }} className="flex items-center gap-2 bg-primary text-primary-foreground px-3 py-2 rounded-xl text-sm font-medium hover:bg-primary/90">
                  <Plus size={14} /> إضافة
                </button>
              </div>
              ...
              {/* Add Item Form */}
              {showItemForm && (
                <FormModal open={showItemForm} title={editItem ? 'تعديل العنصر' : tab === 'vocab' ? 'إضافة مفردة' : tab === 'grammar' ? 'إضافة قاعدة' : 'إضافة محادثة'} onClose={() => setShowItemForm(false)}>
                <div className="bg-card rounded-xl border border-primary/30 p-4 space-y-3">
                  ...
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => { setShowItemForm(false); setEditItem(null); }} className="px-3 py-1.5 rounded-lg text-xs border border-border text-muted-foreground">إلغاء</button>
                    <button onClick={saveItem} className="px-3 py-1.5 rounded-lg text-xs bg-primary text-primary-foreground">{editItem ? 'حفظ التعديل' : 'إضافة'}</button>
                  </div>
                </div>
                </FormModal>
              )}
                  ...
                  <div key={item.id} className="bg-card rounded-xl border border-border p-3 flex items-start gap-2">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button onClick={() => {
                        setEditItem(item);
                        setItemForm({
                          word: item.word || '', meaning: item.meaning || '', example: item.example || '', rule: item.rule || '',
                          title: item.title || '', explanation: item.explanation || '', phrase: item.phrase || '', notes: item.notes || '',
                        });
                        setShowItemForm(true);
                      }} className="p-2 hover:bg-muted rounded text-muted-foreground" aria-label="تعديل">
                        <Edit3 size={12} />
                      </button>
                      <button onClick={() => deleteItem(item.id)} className="p-2 hover:bg-destructive/10 rounded text-muted-foreground hover:text-destructive" aria-label="حذف">
                        <Trash2 size={12} />
                      </button>
                    </div>
                  </div>
