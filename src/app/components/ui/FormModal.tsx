...
export function FormModal({ open, title, onClose, children, panelClassName, layoutId }: FormModalProps) {
  ...
  React.useEffect(() => {
    ...
    const focusPanel = window.requestAnimationFrame(() => {
      ...
      const editable = panel?.querySelector<HTMLElement>('[data-autofocus], input[autofocus], textarea[autofocus], input:not([disabled]), textarea:not([disabled]), select:not([disabled]), [contenteditable="true"]');
      const fallback = panel?.querySelector<HTMLElement>('button:not([disabled]), [href], [tabindex]:not([tabindex="-1"])');
      (editable || fallback || panel)?.focus({ preventScroll: true });
    });
