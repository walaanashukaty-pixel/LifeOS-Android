...
import { countScheduledDates, nextOccurrenceDates, occursOnDate, previousOccurrenceDates, recurrenceLabel, WEEKDAY_OPTIONS } from '../../utils/recurrence';
import { useAuth } from '../App';
import { localDateKey } from '../../utils/ads/reward-policy';
import { parseLocalDateKey } from '../../utils/date';
import { cancelEntityReminders, consumeNotificationDeepLinkTarget, scheduleHabitReminders } from '../../utils/notifications';
...
interface Habit {
  ...
  startDate: string; recurrence: 'daily' | 'weekly' | 'monthly' | 'weekdays';
  repeatDays?: number[]; recurrenceEndDate?: string;
...
function habitOccursOnDate(habit: Habit, date: string): boolean {
  return occursOnDate({
    anchorDate: habit.startDate || habit.createdAt?.slice(0, 10) || todayStr(),
    targetDate: date,
...
function scheduledDatesBack(habit: Habit, throughDate = todayStr(), lookbackDays = 365): string[] {
  return previousOccurrenceDates({
    anchorDate: habit.startDate || habit.createdAt?.slice(0, 10) || throughDate,
    recurrence: habit.recurrence || 'daily',
    repeatDays: habit.repeatDays,
    recurrenceEndDate: habit.recurrenceEndDate,
    throughDate,
...
function calcStreak(habit: Habit): number {
  const scheduled = scheduledDatesBack(habit);
  let streak = 0;
  for (let i = 0; i < scheduled.length; i += 1) {
    const date = scheduled[i];
    const log = habit.logs.find(item => item.date === date);
    // A due occurrence that is still pending today should not destroy yesterday's streak.
    if (i === 0 && date === todayStr() && !log) continue;
...
function calcLongestStreak(habit: Habit): number {
  const scheduled = [...scheduledDatesBack(habit, todayStr(), 1095)].reverse();
  let current = 0;
  let longest = 0;
  for (const date of scheduled) {
    const log = habit.logs.find(item => item.date === date);
    if (date === todayStr() && !log) continue;
    if (log?.completed) {
      current += 1;
...
function habitRate(habit: Habit, days = 30): number {
  const end = new Date();
  const start = new Date();
  start.setDate(start.getDate() - (days - 1));
  const fromDate = localDateKey(start);
  const throughDate = localDateKey(end);
  const opportunities = countScheduledDates({
    anchorDate: habit.startDate || habit.createdAt?.slice(0, 10) || throughDate,
    recurrence: habit.recurrence || 'daily',
    repeatDays: habit.repeatDays,
    recurrenceEndDate: habit.recurrenceEndDate,
  ...
  const completed = habit.logs.filter(log => log.completed && log.date >= fromDate && log.date <= throughDate && habitOccursOnDate(habit, log.date)).length;
  return Math.round((completed / opportunities) * 100);
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <button type="button" aria-label="الفترة السابقة" onClick={prev} className="lifeos-icon-action hover:bg-muted text-muted-foreground"><ChevronRight size={14} /></button>
        <span className="text-xs font-semibold text-foreground">{monthNames[month]} {year}</span>
        <button type="button" aria-label="الفترة التالية" onClick={next} className="lifeos-icon-action hover:bg-muted text-muted-foreground"><ChevronLeft size={14} /></button>
      </div>
      {/* Day labels */}
...
function HabitDetailModal({ habit, onClose, onLog }: {
  ...
  const dueToday = habitOccursOnDate(habit, today);
  const streak = calcStreak(habit);
  const longest = calcLongestStreak(habit);
  ...
  return (
    ...
    >
      <motion.div
        layoutId={`lifeos-habit-card-${habit.id}`}
        className="bg-background rounded-t-3xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[92vh] overflow-hidden flex flex-col shadow-2xl border border-border"
        initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 72, scale: 0.985 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 76, scale: 0.99 }}
        transition={{ type: 'spring', stiffness: 390, damping: 34 }}
        drag={reduceMotion ? false : 'y'}
        dragConstraints={{ top: 0, bottom: 0 }}
        dragElastic={{ top: 0, bottom: 0.32 }}
        onDragEnd={(_, info) => { if (info.offset.y > 120 || info.velocity.y > 900) { hapticWarning(); onClose(); } }}
      ...
      >
        ...
        <div className="flex items-center gap-3 p-5 border-b border-border flex-shrink-0"
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">↻ {recurrenceLabel(habit.recurrence, habit.repeatDays)}</p>
          </div>
          <button onClick={onClose} className="lifeos-fast-close flex h-11 w-11 touch-manipulation items-center justify-center rounded-xl hover:bg-muted text-muted-foreground active:scale-95">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-0 border-b border-border flex-shrink-0">
          {[
            ...
            { label: 'حالة اليوم',     value: !dueToday ? 'راحة' : todayLog?.completed ? '✅' : todayLog ? '❌' : '—', color: 'text-foreground' },
          ].map((s, i) => (
        ...
        <div className="px-5 py-3 border-b border-border flex-shrink-0">
          {!dueToday && <p className="mb-2 text-xs text-muted-foreground">هذه العادة ليست مطلوبة اليوم حسب جدول التكرار.</p>}
...
function HabitCard({ habit, today, index, reduceMotion, onLog, onEdit, onDelete, onToggleFavorite, onOpen }: {
  habit: Habit; today: string; index: number; reduceMotion: boolean;
  ...
  const longest   = calcLongestStreak(habit);
  const totalDone = habit.logs.filter(l => l.completed).length;
  const rate30 = habitRate(habit, 30);
  const dueToday = habitOccursOnDate(habit, today);
  const nextDate = !dueToday
    ? nextOccurrenceDates({
        anchorDate: habit.startDate || habit.createdAt?.slice(0, 10) || today,
        recurrence: habit.recurrence || 'daily',
        repeatDays: habit.repeatDays,
        recurrenceEndDate: habit.recurrenceEndDate,
        fromDate: today,
  ...
  return (
    ...
    <motion.div
      ...
      transition={{ duration: reduceMotion ? 0.1 : 0.24, delay: reduceMotion ? 0 : Math.min(index * 0.03, 0.18), ease: [0.22, 1, 0.36, 1] }}
      className={`lifeos-list-card overflow-hidden border group transition-all ${dueToday ? 'bg-card border-border hover:border-primary/20 hover:shadow-md' : 'bg-muted/20 border-border/70'}`}
    >
      {/* Color bar */}
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">↻ {recurrenceLabel(habit.recurrence, habit.repeatDays)}</p>
            {!dueToday && <p className="text-[10px] text-muted-foreground mt-0.5">غير مطلوبة اليوم حسب جدولها{nextDate ? ` • القادمة ${new Date(nextDate + 'T12:00').toLocaleDateString('ar-SA', { weekday: 'short', month: 'short', day: 'numeric' })}` : ''}</p>}
          </div>
          <div className="flex items-center gap-0.5 flex-shrink-0">
            <motion.button whileTap={reduceMotion ? undefined : { scale: 0.82 }} onClick={() => onToggleFavorite(habit)} className={`p-1.5 rounded-lg transition-colors ${habit.favorite ? 'text-amber-500' : 'text-muted-foreground hover:text-amber-500 opacity-100 md:opacity-0 md:group-hover:opacity-100'}`}>
              <Star size={13} fill={habit.favorite ? 'currentColor' : 'none'} />
            </motion.button>
            <button onClick={() => onEdit(habit)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all">
              <Edit3 size={13} />
            </button>
            <button onClick={() => onDelete(habit.id)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all">
              <Trash2 size={13} />
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="text-center">
            <motion.p
              ...
              animate={!reduceMotion && streak > 0 ? { scale: [1, 1.08, 1] } : { scale: 1 }}
        ...
        <div className="flex gap-0.5 mb-3 overflow-hidden">
          {Array.from({ length: 28 }, (_, i) => {
            ...
            const scheduled = habitOccursOnDate(habit, ds);
            return (
              <div
                key={i}
                title={ds}
                ...
                style={{
                  background: log?.completed ? habit.color
                    : log ? '#ef444444'
                    : scheduled ? 'var(--color-muted)' : 'transparent',
                  opacity: log?.completed ? 1 : log ? 0.7 : scheduled ? 0.3 : 0.15,
                }}
              />
            );
          })}
        ...
        <div className="flex gap-2">
          <motion.button
            ...
            whileTap={!reduceMotion && dueToday ? { scale: 0.95 } : undefined}
          ...
          >
            <motion.span
              className="flex"
              animate={todayLog?.completed && !reduceMotion ? { scale: [1, 1.35, 1], rotate: [0, -10, 0] } : { scale: 1, rotate: 0 }}
              transition={{ duration: 0.34 }}
            ><Check size={13} /></motion.span>
            {!dueToday ? 'ليس اليوم' : todayLog?.completed ? 'تم ✓' : 'أنجزت'}
          </motion.button>
          <motion.button
            disabled={!dueToday}
          ...
          >
            <X size={13} />
            {!dueToday ? 'راحة' : todayLog && !todayLog.completed ? 'فات ✗' : 'فاتني'}
          </motion.button>
        </div>
      </div>
    </motion.div>
      <QuickActionSheet
        open={quickOpen}
        title={habit.name}
        subtitle="إجراءات سريعة للعادة"
        onClose={() => setQuickOpen(false)}
        actions={[
          { id: 'open', label: 'فتح التفاصيل والإحصائيات', icon: <BarChart2 size={16} />, onSelect: () => onOpen(habit) },
          { id: 'done', label: todayLog?.completed ? 'مسجلة كمكتملة اليوم' : 'تسجيل الإنجاز اليوم', icon: <Check size={16} />, disabled: !dueToday || !!todayLog?.completed, onSelect: () => onLog(habit.id, true) },
          { id: 'favorite', label: habit.favorite ? 'إزالة من المفضلة' : 'إضافة للمفضلة', icon: <Star size={16} fill={habit.favorite ? 'currentColor' : 'none'} />, onSelect: () => onToggleFavorite(habit) },
          { id: 'edit', label: 'تعديل العادة', icon: <Edit3 size={16} />, onSelect: () => onEdit(habit) },
          { id: 'delete', label: 'حذف العادة', icon: <Trash2 size={16} />, destructive: true, onSelect: () => onDelete(habit.id) },
        ]}
      />
    </>
  );
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-card rounded-2xl border border-border w-full max-w-lg max-h-[80vh] flex flex-col shadow-2xl">
        <div className="flex items-center gap-3 p-4 border-b border-border flex-shrink-0">
          ...
          <button onClick={onClose} className="lifeos-fast-close flex h-11 w-11 touch-manipulation items-center justify-center rounded-lg hover:bg-muted text-muted-foreground active:scale-95"><X size={18}/></button>
        </div>
...
function HabitForm({ habit, categories, onSave, onCancel }: {
  ...
  const [form, setForm] = useState({
    ...
    recurrence:   (habit?.recurrence || 'daily') as 'daily' | 'weekly' | 'monthly' | 'weekdays',
  ...
  return (
    <div className="bg-card rounded-2xl border border-primary/30 p-5 space-y-4 shadow-xl shadow-primary/5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onCancel} className="lifeos-fast-close flex h-11 w-11 touch-manipulation items-center justify-center rounded-lg hover:bg-muted text-muted-foreground active:scale-95"><X size={18}/></button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-semibold text-foreground mb-1.5">تكرار العادة</label>
          <select value={form.recurrence} onChange={e => setForm(f => ({ ...f, recurrence: e.target.value as any, repeatDays: e.target.value === 'weekdays' ? f.repeatDays : [] }))}
        ...
        <div>
          <label className="block text-sm font-semibold text-foreground mb-1.5">يبدأ من</label>
        ...
        {form.recurrence === 'weekdays' && (
          <div className="md:col-span-2">
            <label className="block text-sm font-semibold text-foreground mb-2">الأيام</label>
            <div className="flex flex-wrap gap-2">
              {WEEKDAY_OPTIONS.map(day => {
                const active = form.repeatDays.includes(day.value);
                return <button key={day.value} type="button" onClick={() => setForm(f => ({ ...f, repeatDays: active ? f.repeatDays.filter(d => d !== day.value) : [...f.repeatDays, day.value] }))}
                  className={`min-w-12 px-3 py-2 rounded-xl text-xs font-semibold border ${active ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground'}`}>{day.short}</button>;
              })}
            </div>
          </div>
        )}
        ...
        <details className="md:col-span-2 rounded-xl border border-border bg-muted/20 px-4 py-3">
          <summary className="cursor-pointer text-xs font-semibold text-foreground">خيارات متقدمة</summary>
          <div className="mt-3 max-w-sm">
            <label className="block text-xs font-medium text-muted-foreground mb-1.5">إيقاف التكرار في (اختياري)</label>
            <input type="date" min={form.startDate || undefined} value={form.recurrenceEndDate} onChange={e => setForm(f => ({ ...f, recurrenceEndDate: e.target.value }))}
              className="w-full bg-input-background border border-border rounded-xl py-2.5 px-4 text-sm" />
          </div>
        </details>
...
type FilterTab = 'all' | 'today' | 'completed' | 'missed' | 'favorites';
...
export function HabitsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const deepLinkId = consumeNotificationDeepLinkTarget('habit');
      if (deepLinkId) window.setTimeout(() => focusDeepLinkedCard(`habit-${deepLinkId}`), 120);
  ...
  async function saveHabit(data: any) {
    ...
    if (!data.startDate) { toast.error('حدد تاريخ بداية العادة'); return; }
    if (data.recurrence === 'weekdays' && (!Array.isArray(data.repeatDays) || data.repeatDays.length === 0)) { toast.error('اختر يومًا واحدًا على الأقل'); return; }
    if (data.recurrenceEndDate && data.recurrenceEndDate < data.startDate) { toast.error('تاريخ إيقاف التكرار يجب أن يكون بعد البداية'); return; }
    try {
      let saved: Habit;
      if (editHabit && editHabit.id !== '__tpl__') {
        saved = await api(`/habits/${editHabit.id}`, { method: 'PUT', body: JSON.stringify(data) });
        setHabits(h => h.map(x => x.id === saved.id ? saved : x));
        if (detailHabit?.id === saved.id) setDetailHabit(saved);
        toast.success('تم التحديث ✓');
      } else {
        if (!await guardCreation({ key: 'habits', currentCount: habits.length })) return;
        saved = await api('/habits', { method: 'POST', body: JSON.stringify(data) });
        setHabits(h => [saved, ...h]);
        toast.success('تمت الإضافة ✓');
      }
      if (user?.id) await scheduleHabitReminders(user.id, saved);
  ...
  async function logHabit(id: string, completed: boolean, note?: string) {
    try {
      const wasCompleted = !!getLogForDate(habits.find(h => h.id === id)?.logs || [], today)?.completed;
      ...
      if (completed) {
        ...
        if (!wasCompleted && !reduceMotion) {
          confetti({ particleCount: 28, spread: 46, startVelocity: 22, scalar: 0.68, origin: { y: 0.8 } });
        }
        toast.success('أحسنت! تم التسجيل ✓');
      } else {
        hapticWarning();
      }
  ...
  async function deleteHabit(id: string) {
    if (!(await confirmAction({ title: 'حذف العادة', description: 'سيتم حذف العادة وسجل الإنجاز المرتبط بها نهائيًا.' }))) return;
    try {
      await api(`/habits/${id}`, { method: 'DELETE' });
      if (user?.id) await cancelEntityReminders(user.id, 'habit', id);
  ...
  async function deleteCat(id: string) {
    if (!(await confirmAction({ title: 'حذف الفئة', description: 'سيتم حذف هذه الفئة المخصصة. العادات نفسها لن تُحذف.' }))) return;
  ...
  const dueTodayHabits = habits.filter(h => habitOccursOnDate(h, today));
  const completedToday  = dueTodayHabits.filter(h => getLogForDate(h.logs, today)?.completed).length;
  const missedToday     = dueTodayHabits.filter(h => { const l = getLogForDate(h.logs, today); return l && !l.completed; }).length;
  const activeHabits    = habits.filter(h => h.logs.some(l => {
    const d = parseLocalDateKey(l.date); const now = parseLocalDateKey(localDateKey());
    return !!d && !!now && (now.getTime() - d.getTime()) / 86400000 <= 30;
  }));
  const allStreaks       = habits.map(h => calcStreak(h));
  const longestCurStreak = Math.max(0, ...allStreaks);
  const bestHabit        = habits[allStreaks.indexOf(longestCurStreak)];
  const overallRate      = habits.length > 0 ? Math.round(habits.reduce((sum, habit) => sum + habitRate(habit, 30), 0) / habits.length) : 0;

  const needsImprovement = [...habits]
    .map(h => ({ h, rate: habitRate(h, 7) }))
    .filter(x => x.rate < 50)
  ...
  const filtered = habits.filter(h => {
    ...
    const dueToday = habitOccursOnDate(h, today);
    if (filterTab === 'completed' && (!dueToday || !todayLog?.completed)) return false;
    if (filterTab === 'missed'    && !(dueToday && todayLog && !todayLog.completed)) return false;
    ...
    if (filterTab === 'today') return dueToday;
    return true;
  });
  ...
  return (
    <div className="lifeos-page-shell max-w-6xl mx-auto space-y-6">

      {/* ── Page introduction ─────────────────────────────────────────────── */}
      <PageHero
        eyebrow="نظام الاستمرارية"
        title="العادات"
        description="تابع ما يهمك اليوم، وحافظ على السلسلة بدون ما تغرق في التفاصيل."
        icon={<Flame size={20} />}
        meta={(
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-primary/10 px-2.5 py-1 text-[10px] font-extrabold text-primary">{dueTodayHabits.length} مطلوبة اليوم</span>
            {longestCurStreak > 0 && <span className="rounded-full bg-orange-500/10 px-2.5 py-1 text-[10px] font-bold text-orange-500">🔥 {longestCurStreak} يوم متتالي</span>}
          </div>
        )}
        actions={(
          <>
            <motion.button whileTap={reduceMotion ? undefined : { scale: 0.97 }} onClick={() => setShowTemplates(true)} className="lifeos-secondary-action">
              <Sparkles size={14} /> قوالب
            </motion.button>
            <motion.button whileTap={reduceMotion ? undefined : { scale: 0.97 }} onClick={() => { setShowForm(true); setEditHabit(null); }} className="lifeos-primary-action">
      ...
      <RewardCapacityBar rewardKey="habits" currentCount={habits.length} />

      {/* ── Today summary ─────────────────────────────────────────────────── */}
      <div className="lifeos-metric-grid">
        <MetricTile label="مطلوبة اليوم" value={dueTodayHabits.length} icon={<Activity size={16}/>} iconClassName="text-blue-500" helper={`${habits.length} عادة إجمالاً`} />
        <MetricTile label="مكتملة اليوم" value={completedToday} icon={<CheckCircle2 size={16}/>} iconClassName="text-emerald-500" helper={dueTodayHabits.length ? `${Math.round((completedToday / dueTodayHabits.length) * 100)}% من عادات اليوم` : 'لا عادات مطلوبة اليوم'} />
        <MetricTile label="أطول سلسلة" value={`${longestCurStreak} يوم`} icon={<Flame size={16}/>} iconClassName="text-orange-500" helper="أفضل استمرارية حالية" />
        <MetricTile label="معدل الإنجاز" value={`${overallRate}%`} icon={<TrendingUp size={16}/>} iconClassName="text-purple-500" progress={overallRate} />
      </div>
      ...
      {/* ── Form ────────────────────────────────────────────────────────────── */}
      {showForm && (
        <FormModal open={showForm} title={editHabit && editHabit.id !== '__tpl__' ? 'تعديل العادة' : 'إضافة عادة جديدة'} onClose={() => { setShowForm(false); setEditHabit(null); }} layoutId={editHabit && editHabit.id !== '__tpl__' ? `lifeos-habit-card-${editHabit.id}` : undefined}>
        <HabitForm
      ...
      {categories.filter(c => c.isCustom).length > 0 && (
        <div className="lifeos-panel p-4">
          <div className="flex items-center justify-between mb-3">
      ...
      <div className="space-y-3">
        <div className="lifeos-toolbar flex flex-col gap-2 sm:flex-row">
          ...
          <button onClick={() => setShowFilters(s => !s)}
            className={`lifeos-icon-action ${showFilters ? '!border-primary !bg-primary/5 !text-primary' : ''}`}>
            <SlidersHorizontal size={15} />
          </button>
          <button onClick={() => { setEditCat(null); setShowCatForm(true); }}
            className="lifeos-secondary-action">
            <Plus size={14} /> فئة
          </button>
        </div>
        ...
        <div className="lifeos-segmented">
          {TABS.map(tab => {
            ...
            return (
              <button key={tab.key} data-active={active} onClick={() => setFilterTab(tab.key)} className="lifeos-segmented-item">
                {active && <motion.span layoutId="lifeos-habit-filter-active" className="lifeos-segmented-indicator" transition={{ type: 'spring', stiffness: 500, damping: 38 }} />}
      ...
      {loading ? (
        <div className="lifeos-panel flex flex-col items-center justify-center gap-3 py-16">
          ...
          <div className="text-center">
            <p className="text-sm font-bold text-foreground">نراجع استمراريتك</p>
            <p className="mt-1 text-xs text-muted-foreground">لحظة واحدة لعرض عادات اليوم...</p>
          </div>
        </div>
      ) : sorted.length === 0 ? (
        <AnimatedEmptyState
          icon={Flame}
          title={habits.length === 0 ? 'لا توجد عادات بعد' : 'لا نتائج'}
          description={habits.length === 0 ? 'ابدأ بعادة بسيطة، وسيتحوّل الاستمرار إلى تقدم مرئي يومًا بعد يوم.' : 'حاول تغيير الفلاتر أو البحث للوصول إلى العادة المطلوبة.'}
          action={habits.length === 0 ? (
            <div className="flex justify-center gap-2">
              <motion.button whileTap={reduceMotion ? undefined : { scale: 0.96 }} onClick={() => setShowTemplates(true)}
                className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm text-foreground">
                <Sparkles size={14} /> من القوالب
              </motion.button>
              <motion.button whileTap={reduceMotion ? undefined : { scale: 0.96 }} onClick={() => setShowForm(true)}
