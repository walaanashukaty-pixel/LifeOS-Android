...
export function AuthPage({ onSuccess }: AuthPageProps) {
  ...
  return (
    <main className="lifeos-auth-shell" dir="rtl">
      ...
      <div className="lifeos-auth-mobile-brand md:hidden" aria-label="LifeOS">
        <div className="lifeos-auth-brand !mb-0 !h-11 !w-11 !rounded-2xl"><span>LO</span></div>
        <div><strong>LifeOS</strong><span>نظام إدارة حياتك</span></div>
      </div>
      ...
      <motion.section className="lifeos-auth-card" initial={reduceMotion ? false : { opacity: 0, y: 16, scale: .99 }} animate={{ opacity: 1, y: 0, scale: 1 }}>
        <div className="mb-5 flex items-start justify-between gap-3">
          <div><p className="lifeos-eyebrow">{mode === 'login' ? 'تسجيل الدخول' : 'حساب جديد'}</p><h2 className="text-xl font-black text-foreground">{mode === 'login' ? 'ادخل إلى LifeOS' : 'أنشئ حساب LifeOS'}</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">{mode === 'login' ? 'استخدم بريدك الإلكتروني وكلمة المرور، أو تابع بحساب Google.' : 'أنشئ حسابًا بالبريد الإلكتروني وكلمة المرور، أو استخدم Google.'}</p></div>
        ...
        <div className="lifeos-segmented mb-5" role="tablist" aria-label="نوع الدخول">
          {([['login', 'تسجيل الدخول'], ['signup', 'حساب جديد']] as const).map(([id, label]) => (
        ...
        <div className="my-5 flex items-center gap-3" aria-hidden="true"><div className="h-px flex-1 bg-border" /><span className="text-[10px] font-bold text-muted-foreground">أو</span><div className="h-px flex-1 bg-border" /></div>
        <button type="button" disabled={busy} onClick={handleGoogleSignIn} className="lifeos-auth-google">
          ...
          <span>{mode === 'login' ? 'الدخول باستخدام Google' : 'إنشاء الحساب باستخدام Google'}</span>
        </button>
