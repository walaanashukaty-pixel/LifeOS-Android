...
export function EventsPage() {
  ...
  async function saveEvent() {
    ...
    try {
      if (editEvent) {
        const updated = await api(`/events/${editEvent.id}`, { method: 'PUT', body: JSON.stringify(form) });
        setEvents(e => e.map(x => x.id === updated.id ? updated : x));
        toast.success('تم التحديث');
      } else {
        const created = await api('/events', { method: 'POST', body: JSON.stringify(form) });
...
function EventCard({ event, onEdit, onDelete }: { event: any; onEdit: (e: any) => void; onDelete: (id: string) => void }) {
  ...
  return (
    <div className={`bg-card rounded-xl border p-4 ${isToday ? 'border-primary/30' : 'border-border'}`}>
