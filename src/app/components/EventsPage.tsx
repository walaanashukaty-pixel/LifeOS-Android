  ...
  async function saveEvent() {
    ...
    try {
      ...
      if (editEvent) {
        ...
        const upcomingCount = events.filter(event => event.date >= currentToday).length;
        if (form.date >= currentToday && !await guardCreation({ key: 'events', currentCount: upcomingCount })) return;
