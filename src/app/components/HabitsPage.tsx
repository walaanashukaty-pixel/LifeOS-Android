...
function HabitCard({ habit, today, onLog, onEdit, onDelete, onToggleFavorite, onOpen }: {
  ...
  return (
    <div id={`habit-${habit.id}`} className={`rounded-2xl border overflow-hidden group transition-all ${dueToday ? 'bg-card border-border hover:border-primary/20 hover:shadow-sm' : 'bg-muted/20 border-border/70'}`}>
      {/* Color bar */}
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex items-center gap-0.5 flex-shrink-0">
            <button onClick={() => onToggleFavorite(habit)} className={`p-1.5 rounded-lg transition-colors ${habit.favorite ? 'text-amber-500' : 'text-muted-foreground hover:text-amber-500 opacity-100 md:opacity-0 md:group-hover:opacity-100'}`}>
...
export function HabitsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      {/* ── Form ────────────────────────────────────────────────────────────── */}
      {showForm && (
        <FormModal open={showForm} title={editHabit && editHabit.id !== '__tpl__' ? 'تعديل العادة' : 'إضافة عادة جديدة'} onClose={() => { setShowForm(false); setEditHabit(null); }}>
        <HabitForm
