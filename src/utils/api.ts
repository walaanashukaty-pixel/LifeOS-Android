...
const RETRYABLE_READ_STATUSES = new Set([408, 502, 503, 504]);
let pendingNetworkRequests = 0;

function emitNetworkActivity(delta: number) {
  pendingNetworkRequests = Math.max(0, pendingNetworkRequests + delta);
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('lifeos:network-activity', { detail: { pending: pendingNetworkRequests } }));
  }
}
...
async function fetchWithTimeout(path: string, options: RequestInit): Promise<Response> {
  ...
  const timeout = window.setTimeout(() => controller.abort(new DOMException('Request timeout', 'TimeoutError')), REQUEST_TIMEOUT_MS);
  emitNetworkActivity(1);
  try {
...
function networkErrorMessage(error: unknown): string {
  ...
  if (/timeout|timed out|aborted/i.test(raw)) {
    return 'الاتصال بالخادم بطيء جدًا. تحقق من الإنترنت ثم حاول مجددًا.';
  }
...
async function rawAuthorizedRequest<T>(
  ...
  const canRetryRead = retryNetwork && (method === 'GET' || method === 'HEAD');
  try {
    ...
    if (response.status === 401 && retryAuth) {
      ...
      if (refreshed.data.session) {
        return rawAuthorizedRequest<T>(path, options, refreshed.data.session, false, retryNetwork);
      }
    }

    if (canRetryRead && RETRYABLE_READ_STATUSES.has(response.status)) {
      await sleep(650);
      return rawAuthorizedRequest<T>(path, options, session, retryAuth, false);
    }

    return await parseResponse<T>(response);
  } catch (error) {
    const status = Number((error as Error & { status?: number })?.status || 0);
    if (canRetryRead && !status && !(options.signal?.aborted)) {
      await sleep(650);
      return rawAuthorizedRequest<T>(path, options, session, retryAuth, false);
    }
