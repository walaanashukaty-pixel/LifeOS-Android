...
export function Layout({ page, setPage, children, onRefresh }: LayoutProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('lifeos:keyboard-visibility', onKeyboardVisibility);
    return () => window.removeEventListener('lifeos:keyboard-visibility', onKeyboardVisibility);
  }, []);
  ...
  return (
    <div className="lifeos-app-shell flex overflow-hidden bg-background selection:bg-primary/20" dir="rtl">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <motion.header
          className={`lifeos-app-header sticky top-0 z-30 flex flex-shrink-0 items-center gap-2.5 border-b px-3 backdrop-blur-xl transition-[height,box-shadow,background-color] duration-150 md:h-14 md:gap-3 md:px-6 ${headerCompact ? 'h-[48px] border-border/90 bg-card/[0.98] shadow-[0_8px_22px_rgba(0,0,0,0.05)]' : 'h-[52px] border-border bg-card/95'}`}
        ...
        >
          ...
          >
            <h2 className="truncate text-sm font-bold text-foreground md:text-base md:font-semibold">{page === 'dashboard' ? <><span className="md:hidden">LifeOS</span><span className="hidden md:inline">{PAGE_TITLES[page]}</span></> : PAGE_TITLES[page]}</h2>
        ...
        <main
          ...
          className="lifeos-main lifeos-scroll-region min-h-0 flex-1 overflow-y-auto px-3 pb-28 pt-3 sm:px-4 md:p-7"
          onTouchStart={handleMainTouchStart}
          onTouchEnd={handleMainTouchEnd}
          onScroll={handleMainScroll}
        >
          <PullToRefresh onRefresh={onRefresh} disabled={keyboardOpen}>
            <AnimatePresence mode="wait" initial={false}>
              <motion.div
                key={page}
                initial={isMobile || reduceMotion ? false : { opacity: 0, y: 8, scale: 0.997 }}
                animate={{ opacity: 1, y: 0, scale: 1 }}
                exit={isMobile || reduceMotion ? { opacity: 1 } : { opacity: 0, y: -4, scale: 0.999 }}
                transition={{ duration: isMobile ? 0 : reduceMotion ? 0.08 : 0.2, ease: [0.22, 1, 0.36, 1] }}
              >
                {children}
              </motion.div>
            </AnimatePresence>
