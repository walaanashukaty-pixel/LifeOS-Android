...
test('Pro catalog promises the implemented ad-free and higher-or-unlimited capacity benefits without inventing limits', async () => {
  const source = await readFile('src/app/agent/pro-features.ts', 'utf8');
  assert.match(source, /بدون إعلانات/);
  assert.match(source, /حدود أعلى أو غير محدودة حسب نظام الاشتراك الحالي/);
});
