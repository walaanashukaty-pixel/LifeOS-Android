...
export function AgreementsPage() {
  ...
  function resetForm() {
    setForm({ personName: '', type: 'وعد', details: '', agreementDate: localDateKey(), dueDate: '', status: 'active' });
    setEditItem(null);
    setShowForm(false);
  }
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map(agreement => {
            ...
            const daysUntil = due && todayDate ? Math.ceil((due.getTime() - todayDate.getTime()) / 86400000) : null;
            return (
