  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="rounded-xl border border-border bg-muted/30 px-4 py-3 text-xs leading-5 text-muted-foreground">
        الهدف لا يتكرر يوميًا؛ يبقى في قائمة الأهداف إلى أن تكمله أو تحذفه. الموعد النهائي اختياري فقط إذا كان للهدف تاريخ استحقاق.
      </div>
      ...
      {/* Form */}
      {showForm && (
        <FormModal open={showForm} title={editGoal ? 'تعديل الهدف' : 'إضافة هدف جديد'} onClose={resetForm}>
                  ...
                  <div key={goal.id} className={`bg-card rounded-xl border p-4 ${isComplete ? 'border-primary/30' : 'border-border'}`}>
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex gap-1 mr-2">
                        <button onClick={() => { setEditGoal(goal); setForm({ title: goal.title, description: goal.description || '', type: goal.type, startDate: '', deadline: goal.deadline || '', progress: goal.progress || 0 }); setShowForm(true); }} className="p-1 rounded hover:bg-muted text-muted-foreground">
                          <Edit3 size={12} />
                        </button>
