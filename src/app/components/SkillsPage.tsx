...
export function SkillsPage() {
  ...
  async function saveSkill() {
    const validationError = firstValidation(
      textValidation(form.name, 'اسم المهارة', 120),
      textValidation(form.notes, 'الملاحظات', 3000, false),
      numberValidation(form.currentLevel, 'المستوى الحالي', { min: 1, max: 10 }),
      numberValidation(form.targetLevel, 'المستوى المستهدف', { min: 1, max: 10 }),
    );
    if (validationError) { toast.error(validationError); return; }
    if (Number(form.targetLevel) < Number(form.currentLevel)) { toast.error('المستوى المستهدف يجب ألا يكون أقل من المستوى الحالي'); return; }
    try {
      if (editSkill) {
  ...
  async function deleteSkill(id: string) {
    if (!(await confirmAction({ title: 'حذف المهارة', description: 'سيتم حذف هذه المهارة وبيانات التقدم المرتبطة بها. لا يمكن التراجع عن هذا الإجراء.' }))) return;
    try {
  ...
  const averageLevel = skills.length ? (skills.reduce((sum, sk) => sum + Number(sk.currentLevel || 0), 0) / skills.length).toFixed(1) : '0';
  const targetGap = skills.length ? Math.max(0, Math.round(skills.reduce((sum, sk) => sum + Math.max(0, Number(sk.targetLevel || 0) - Number(sk.currentLevel || 0)), 0) / skills.length)) : 0;

  if (loading) return <LoadingPage label="جارٍ تجهيز خريطة المهارات" />;

  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <PageHero eyebrow="SKILL GROWTH" title="تطوير المهارات" description="راقب وقت التدريب ومستواك الحالي والفجوة نحو المستوى الذي تريد الوصول إليه." icon={<Zap size={20} />} actions={<button onClick={() => { resetForm(); setShowForm(true); }} className="lifeos-primary-action"><Plus size={16} /> مهارة جديدة</button>} meta={<div className="lifeos-v3-hero-meta"><span className="lifeos-chip">{skills.length} مهارة</span><span className="lifeos-chip">{totalHours} ساعة تدريب</span><span className="lifeos-chip">متوسط {averageLevel}/10</span></div>} />

      <div className="lifeos-v3-summary-grid">
        <SummaryCard value={skills.length} label="المهارات المتتبعة" />
        <SummaryCard value={`${totalHours}س`} label="وقت الاستثمار" />
        <SummaryCard value={`${averageLevel}/10`} label="متوسط المستوى" />
        <SummaryCard value={targetGap} label="متوسط الفجوة للهدف" />
      </div>

      {/* Radar Chart */}
      {radarData.length > 2 && (
        <div className="lifeos-panel p-5">
          <SectionHeading title="خريطة المهارات" description="صورة سريعة عن توازن مستوياتك الحالية" />
          <RadarChart data={radarData} />
        </div>
      )}

      <RewardCapacityBar rewardKey="skills" currentCount={skills.length} />

      {/* Form */}
      {showForm && (
        <FormModal open={showForm} title={editSkill ? 'تعديل المهارة' : 'إضافة مهارة جديدة'} onClose={resetForm}>
      ...
      {/* Skills Grid */}
      {skills.length === 0 ? (
        <AnimatedEmptyState icon={Zap} title="ابنِ خريطة مهاراتك" description="أضف أول مهارة وحدد مستواك الحالي والمستهدف، وبعدها تتبع الساعات والتطور." action={<button onClick={() => { resetForm(); setShowForm(true); }} className="lifeos-primary-action"><Plus size={15} /> إضافة مهارة</button>} />
      ) : (
              ...
              <div key={skill.id} className="bg-card rounded-xl border border-border p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-1">
                    <button type="button" aria-label={`تعديل ${skill.name}`} onClick={() => { setEditSkill(skill); setForm({ name: skill.name, category: skill.category, currentLevel: skill.currentLevel, targetLevel: skill.targetLevel, hoursInvested: skill.hoursInvested || 0, notes: skill.notes || '' }); setShowForm(true); }} className="lifeos-icon-action hover:bg-muted text-muted-foreground">
                      <Edit3 size={12} />
                    </button>
                    <button type="button" aria-label={`حذف ${skill.name}`} onClick={() => deleteSkill(skill.id)} className="lifeos-icon-action hover:bg-destructive/10 text-muted-foreground hover:text-destructive">
                      <Trash2 size={12} />
                    </button>
                  </div>
                </div>
