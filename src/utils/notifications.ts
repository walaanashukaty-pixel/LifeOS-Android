...
import { buildReminderOccurrences, notificationNumericId, type ReminderEntityType, type ReminderRecurrence } from './notification-model';
...
export type NotificationRecordEntityType = ReminderEntityType | 'smart';

export interface LifeOSNotificationRecord {
  key: string;
  nativeId: number;
  entityType: NotificationRecordEntityType;
  entityId: string;
  page: Page;
  title: string;
  body: string;
...
async function scheduleEntity({
  ...
  const occurrences = buildReminderOccurrences({
    entityType, entityId, title, date, time, recurrence, repeatDays, recurrenceEndDate, leadMinutes, horizonDays: 30,
  });
...
function recurrenceOf(value: unknown): ReminderRecurrence {
  return value === 'daily' || value === 'weekly' || value === 'monthly' || value === 'yearly' || value === 'weekdays' ? value : 'once';
}
...
export async function scheduleTaskReminders(userId: string, task: any): Promise<number> {
  ...
  return scheduleEntity({
    ...
    repeatDays: Array.isArray(task?.repeatDays) ? task.repeatDays : undefined,
...
export async function scheduleHabitReminders(userId: string, habit: any): Promise<number> {
  return scheduleEntity({
    ...
    date: habit?.startDate || localDateKey(),
    time: habit?.reminderTime || '',
    recurrence: recurrenceOf(habit?.recurrence || 'daily'),
    repeatDays: Array.isArray(habit?.repeatDays) ? habit.repeatDays : undefined,
    recurrenceEndDate: habit?.recurrenceEndDate || undefined,
  });
}
...
export async function scheduleEventReminder(userId: string, event: any): Promise<number> {
  ...
  return scheduleEntity({
    ...
    date: event?.date || '',
    time: event?.time || '',
    recurrence: recurrenceOf(event?.recurrence),
    repeatDays: Array.isArray(event?.repeatDays) ? event.repeatDays : undefined,
    recurrenceEndDate: event?.recurrenceEndDate || undefined,
    leadMinutes,
  });
}

export async function cancelSmartNotifications(userId: string): Promise<void> {
  const records = getNotificationRecords(userId);
  const removed = records.filter(record => record.entityType === 'smart');
  await cancelNativeIds(removed.map(record => record.nativeId));
  saveRecords(userId, records.filter(record => record.entityType !== 'smart'));
}

export async function scheduleSmartNotification(userId: string, input: {
  key: string;
  title: string;
  body: string;
  scheduledFor: string;
  ...
  if (!userId || !input.key || !input.scheduledFor) return 0;
  const at = new Date(input.scheduledFor);
  if (Number.isNaN(at.getTime()) || at.getTime() <= Date.now()) return 0;
  const records = getNotificationRecords(userId);
  if (records.some(record => record.entityType === 'smart' && record.entityId === input.key)) return 0;

  const permission = await requestNotificationPermission();
  if (isNative() && permission !== 'granted') return 0;
  const nativeId = notificationNumericId('smart', userId, input.key);
  const page = input.page || 'ai';
  if (isNative()) {
    try {
      await LocalNotifications.schedule({
        notifications: [{
          ...
          autoCancel: true,
          group: 'lifeos-smart',
          extra: { page, entityType: 'smart', entityId: input.key, nativeId },
        }],
      });
    } catch {
      return 0;
...
export async function setupNotificationDeepLinkListener(
  ...
  try {
    const handle = await LocalNotifications.addListener('localNotificationActionPerformed', action => {
      ...
      const entityType = action.notification.extra?.entityType as NotificationRecordEntityType | undefined;
      const entityId = action.notification.extra?.entityId as string | undefined;
      if ((entityType === 'task' || entityType === 'habit' || entityType === 'event') && entityId) setNotificationDeepLinkTarget(entityType, entityId);
