...
import {
  ...
  Smartphone, ChevronLeft, UserRound, Mail, Crown, Loader2, Sparkles, Shield, Brain, Trash2, Clock3, SlidersHorizontal, Palette, BadgeCheck,
} from 'lucide-react';
import {
...
import { PageHero } from './ui/LifeOSPrimitives';
...
export function AccountPage() {
  ...
  return (
    <div className="lifeos-page-shell mx-auto max-w-3xl space-y-5">
      <PageHero
        eyebrow="الحساب والإعدادات"
        title={userName}
        description={user?.email || 'حساب LifeOS'}
        icon={<UserRound size={21} />}
        meta={<div className="flex flex-wrap gap-2"><span className="lifeos-chip"><BadgeCheck size={12} /> {subscription.isPro ? 'LifeOS Pro' : proPreviewActive ? 'معاينة Pro' : 'LifeOS Free'}</span><span className="lifeos-chip"><Bell size={12} /> {permission === 'granted' ? 'التنبيهات مفعّلة' : 'راجع إذن التنبيهات'}</span><span className="lifeos-chip"><Palette size={12} /> {darkMode ? 'الوضع الداكن' : 'الوضع الفاتح'}</span></div>}
        actions={<button onClick={toggleDark} className="lifeos-secondary-action">{darkMode ? <Sun size={15} /> : <Moon size={15} />} تغيير المظهر</button>}
      />

      <section className="lifeos-plan-card overflow-hidden">
        <div className="flex items-center gap-4 bg-gradient-to-l from-amber-400/10 via-primary/5 to-transparent p-5">
          ...
          {subscriptionLoading ? (
            ...
            <button onClick={stopProPreview} className="lifeos-secondary-action text-primary">إيقاف المعاينة</button>
          ) : (
            <button onClick={openPro} className="lifeos-primary-action">الترقية</button>
          )}
        </div>
      </section>

      <div className="lifeos-settings-overview">
        <div><SlidersHorizontal size={16} className="text-primary" /><span><strong>تحكم مركزي</strong><small>كل إعدادات LifeOS من مكان واحد</small></span></div>
        <div><ShieldCheck size={16} className="text-emerald-500" /><span><strong>خصوصية واضحة</strong><small>AI والتنبيهات تحت سيطرتك</small></span></div>
        <div><Smartphone size={16} className="text-blue-500" /><span><strong>جاهز للموبايل</strong><small>التذكيرات تعمل محليًا على Android</small></span></div>
      </div>
      ...
      <button onClick={logout} className="lifeos-danger-action"><LogOut size={17} /> تسجيل الخروج</button>
...
function SettingsSection({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return <section className="lifeos-settings-section"><div className="lifeos-settings-heading"><span className="lifeos-settings-heading-icon">{icon}</span><span>{title}</span></div><div className="divide-y divide-border/60">{children}</div></section>;
}

function SettingRow({ icon, title, subtitle, action }: { icon: React.ReactNode; title: string; subtitle: string; action: React.ReactNode }) {
  return <div className="lifeos-setting-row"><div className="lifeos-setting-icon">{icon}</div><div className="min-w-0 flex-1"><p className="text-sm font-bold text-foreground">{title}</p><p className="mt-0.5 text-[11px] leading-5 text-muted-foreground">{subtitle}</p></div><div className="flex-shrink-0">{action}</div></div>;
}

function Toggle({ checked, onChange, disabled = false }: { checked: boolean; onChange: (next: boolean) => void; disabled?: boolean }) {
  return <button type="button" role="switch" aria-checked={checked} aria-label="تبديل الإعداد" disabled={disabled} onClick={() => onChange(!checked)} className={`lifeos-switch-target relative h-7 w-12 rounded-full transition-colors disabled:opacity-40 ${checked ? 'bg-primary' : 'bg-muted-foreground/25'}`}><span aria-hidden="true" className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow-sm transition-all ${checked ? 'left-1' : 'left-6'}`} /></button>;
}
