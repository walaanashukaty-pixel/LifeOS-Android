...
import { ArrowRight, Bot, RotateCcw, Send, ShieldCheck, Sparkles } from 'lucide-react';
...
export function LifeOSAgentPage({
  ...
  }, [messages, busy, pendingActions.length, reduceMotion]);
  ...
  return (
    <div className="lifeos-agent-shell mx-auto flex max-w-4xl flex-col gap-4 pb-4">
      ...
      <div className="lifeos-agent-header flex items-center gap-3">
      ...
      <div className="lifeos-agent-conversation min-h-[46vh] space-y-3 overflow-hidden p-4">
        <AnimatePresence initial={false}>
          {messages.map(message => (
            ...
            >
              <div className="max-w-[92%]">
                <motion.div
                  className={`lifeos-agent-bubble whitespace-pre-wrap px-4 py-3 text-sm leading-6 ${message.role === 'user' ? 'lifeos-agent-user' : message.role === 'system' ? 'lifeos-agent-system' : 'lifeos-agent-assistant'}`}
          ...
          {busy && (
            ...
            >
              <div className="flex items-center gap-3 rounded-2xl bg-muted px-4 py-3 text-xs text-muted-foreground">
                <span className="flex items-center gap-1" aria-label="LifeOS يفكر">
                  {[0, 1, 2].map(index => (
                    <motion.span
                      key={index}
                      className="h-1.5 w-1.5 rounded-full bg-primary"
                      animate={reduceMotion ? undefined : { y: [0, -4, 0], opacity: [0.45, 1, 0.45] }}
                      transition={{ duration: 0.8, repeat: Infinity, delay: index * 0.13, ease: 'easeInOut' }}
                    />
                  ))}
                </span>
                <span>عم رتّب طلبك...</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
            ...
            <AnimatePresence initial={false}>
              {pendingActions.map((action, index) => (
                <motion.div
                  ...
                  transition={{ duration: 0.25, delay: reduceMotion ? 0 : Math.min(index * 0.04, 0.16) }}
                >
                  <ActionPreviewCard
                    action={action}
                    busy={!!actionBusyId}
      ...
      {messages.length <= 1 && activeMode === 'chat' && (
        <div className="flex flex-wrap gap-2">
          {SUGGESTED_PROMPTS.map(prompt => <motion.button key={prompt} whileTap={reduceMotion ? undefined : { scale: 0.96 }} whileHover={reduceMotion ? undefined : { y: -1 }} onClick={() => void send(prompt)} className="lifeos-chip px-3 py-2 text-[11px] font-semibold">{prompt}</motion.button>)}
        </div>
      )}

      <div className="lifeos-agent-composer relative z-10 flex items-end gap-2 p-2 md:sticky md:bottom-3">
        <textarea
          value={input}
        ...
        <motion.button
          ...
          animate={!reduceMotion && input.trim() && !busy ? { scale: [1, 1.04, 1] } : undefined}
          transition={{ duration: 0.28 }}
          className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-primary text-white shadow-sm shadow-primary/20 disabled:opacity-40"
        ><Send size={17} /></motion.button>
      </div>
    </div>
  );
}
...
function friendlyAgentError(message: string) {
  ...
  if (message.includes('OPENAI_API_KEY_MISSING') || message.includes('AI_PROVIDER_UNCONFIGURED')) return 'محرك OpenAI غير مربوط بالسيرفر بعد. ما تم تنفيذ أي تغيير.';
  if (message.includes('AI_PROVIDER_FAILED')) return 'تعذر الحصول على رد من OpenAI الآن. ما تم تنفيذ أي تغيير.';
