...
import { Plus, Trash2, Edit3, Dumbbell, Loader2 } from 'lucide-react';
...
export function FitnessPage() {
  ...
  const [form, setForm] = useState({ type: 'المشي', date: localDateKey(), duration: 30, calories: 0, notes: '' });
  const [weightForm, setWeightForm] = useState({ weight: '', date: localDateKey() });
  ...
  async function saveWorkout() {
    ...
    try {
      if (editWorkout) {
        const updated = await api(`/workouts/${editWorkout.id}`, { method: 'PUT', body: JSON.stringify(form) });
        setWorkouts(w => w.map(item => item.id === editWorkout.id ? updated : item));
      ...
      setEditWorkout(null);
      setForm({ type: 'المشي', date: localDateKey(), duration: 30, calories: 0, notes: '' });
      setShowForm(false);
  ...
  async function saveWeight() {
    ...
    try {
      ...
      setWeightForm({ weight: '', date: localDateKey() });
  ...
  async function deleteWorkout(id: string) {
    if (!(await confirmAction({ title: 'حذف التمرين', description: 'سيتم حذف هذا التمرين من سجل اللياقة.' }))) return;
    try {
  ...
  const monthWorkouts = workouts.filter(w => w.date?.startsWith(thisMonthKey)).length;
  const latestWeight = weights.length ? weights[weights.length - 1]?.weight : null;

  if (loading) return <LoadingPage label="جارٍ تجهيز لوحة اللياقة" />;

  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <PageHero eyebrow="FITNESS PULSE" title="اللياقة البدنية" description="شاهد نشاطك، وقت التدريب وتغيّر الوزن كنبض واحد بدل أرقام منفصلة." icon={<Dumbbell size={20} />} actions={<><button onClick={() => setShowWeightForm(true)} className="lifeos-secondary-action">الوزن</button><button onClick={() => { setEditWorkout(null); setForm({ type: 'المشي', date: localDateKey(), duration: 30, calories: 0, notes: '' }); setShowForm(true); }} className="lifeos-primary-action"><Plus size={16} /> تمرين جديد</button></>} meta={<div className="lifeos-v3-hero-meta"><span className="lifeos-chip">{monthWorkouts} جلسة هذا الشهر</span>{latestWeight && <span className="lifeos-chip">آخر وزن {latestWeight} كجم</span>}</div>} />

      <div className="lifeos-v3-summary-grid">
        <SummaryCard value={workouts.length} label="إجمالي الجلسات" />
        <SummaryCard value={`${Math.round(totalHours / 60)}س`} label="وقت التدريب" />
        <SummaryCard value={totalCalories.toLocaleString()} label="السعرات المسجلة" />
        <SummaryCard value={latestWeight ? `${latestWeight} كجم` : '—'} label="آخر وزن" />
      </div>

      {/* Forms */}
      {showForm && (
        <FormModal open={showForm} title={editWorkout ? 'تعديل التمرين' : 'إضافة تمرين'} onClose={() => { setShowForm(false); setEditWorkout(null); }}>
        <div className="bg-card rounded-2xl border border-primary/30 p-5 space-y-4">
          <h3 className="font-semibold text-foreground">{editWorkout ? 'تعديل التمرين' : 'تمرين جديد'}</h3>
          ...
          <div className="flex gap-3 justify-end">
            <button onClick={() => { setShowForm(false); setEditWorkout(null); }} className="px-4 py-2 rounded-xl text-sm border border-border text-muted-foreground">إلغاء</button>
            <button onClick={saveWorkout} className="px-4 py-2 rounded-xl text-sm bg-primary text-primary-foreground">{editWorkout ? 'حفظ التعديل' : 'حفظ'}</button>
          </div>
        </div>
        </FormModal>
      )}
      ...
      {showWeightForm && (
        <FormModal open={showWeightForm} title={'تسجيل الوزن'} onClose={() => setShowWeightForm(false)}>
      ...
      <div className="lifeos-panel p-5">
        <SectionHeading title="سجل التمارين" description="أحدث الجلسات والنشاطات المسجلة" />
        {workouts.length === 0 ? (
          <AnimatedEmptyState icon={Dumbbell} title="ابدأ أول تمرين" description="سجّل أي نشاط ولو كان مشيًا قصيرًا، وبعدها ستبدأ الرسوم والنبض الأسبوعي بالظهور." action={<button onClick={() => { setEditWorkout(null); setForm({ type: 'المشي', date: localDateKey(), duration: 30, calories: 0, notes: '' }); setShowForm(true); }} className="lifeos-primary-action"><Plus size={15} /> تسجيل تمرين</button>} />
        ) : (
            ...
            {[...workouts].sort((a, b) => b.date?.localeCompare(a.date || '') || 0).map(w => (
              <div key={w.id} className="flex items-center gap-3 py-2.5 border-b border-border last:border-0">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{new Date(w.date + 'T12:00:00').toLocaleDateString('ar-SA')}</p>
                </div>
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => { setEditWorkout(w); setForm({ type: w.type || 'المشي', date: w.date || localDateKey(), duration: Number(w.duration || 30), calories: Number(w.calories || 0), notes: w.notes || '' }); setShowForm(true); }} className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground" aria-label="تعديل التمرين">
                    <Edit3 size={13} />
                  </button>
                  <button onClick={() => deleteWorkout(w.id)} className="p-2 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive" aria-label="حذف التمرين">
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
