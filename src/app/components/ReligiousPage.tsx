...
export function ReligiousPage() {
  ...
  const [editDhikr, setEditDhikr] = useState<any | null>(null);
  ...
  const [memForm, setMemForm] = useState({ juz: 1, surah: '', percentage: 0, lastReview: localDateKey() });
  ...
  async function addDhikr() {
    ...
    try {
      if (editDhikr) {
        const updated = await api(`/dhikr/${editDhikr.id}`, { method: 'PUT', body: JSON.stringify(dhikrForm) });
        setDhikrList(d => d.map(item => item.id === editDhikr.id ? updated : item));
  ...
  async function deleteDhikr(id: string) {
    if (!(await confirmAction({ title: 'حذف الذكر', description: 'سيتم حذف هذا الذكر وعدّاده الحالي من سجل التقدم.' }))) return;
    try {
  ...
  async function saveMem() {
    ...
    try {
      if (editMem) {
        const updated = await api(`/memorization/${editMem.id}`, { method: 'PUT', body: JSON.stringify(memForm) });
        setMemorization(m => m.map(item => item.id === editMem.id ? updated : item));
        toast.success('تم تعديل سجل الحفظ');
      } else {
      ...
      setMemForm({ juz: 1, surah: '', percentage: 0, lastReview: localDateKey() });
    } catch { toast.error('فشل الحفظ'); }
  }
  ...
  async function saveLesson() {
    ...
    try {
      if (editLesson) {
        const updated = await api(`/lessons/${editLesson.id}`, { method: 'PUT', body: JSON.stringify(lessonForm) });
        setLessons(l => l.map(item => item.id === editLesson.id ? updated : item));
  ...
  async function deleteLesson(id: string) {
    if (!(await confirmAction({ title: 'حذف الدرس', description: 'سيتم حذف هذا الدرس من قائمة الدروس المحفوظة.' }))) return;
    try {
  ...
  const todayPages = (quranData?.sessions || []).filter((s: any) => (s.localDate || s.date?.slice?.(0, 10)) === localDateKey()).reduce((s: number, r: any) => s + (r.pages || 0), 0);
  ...
  const completedDhikr = dhikrList.filter(d => Number(d.count || 0) >= Number(d.targetCount || 0) && Number(d.targetCount || 0) > 0).length;
  const averageMemorization = memorization.length ? Math.round(memorization.reduce((sum, item) => sum + Number(item.percentage || 0), 0) / memorization.length) : 0;

  if (loading) return <LoadingPage label="جارٍ تجهيز مساحة التقدم الديني" />;

  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <PageHero eyebrow="SPIRITUAL RHYTHM" title="التقدم الديني" description="تابع الأذكار والقراءة والحفظ والدروس ضمن مساحة هادئة وواضحة بدون ازدحام بصري." icon={<BookOpen size={20} />} meta={<div className="lifeos-v3-hero-meta"><span className="lifeos-chip">{todayPages} صفحات اليوم</span><span className="lifeos-chip">{completedDhikr}/{dhikrList.length} أذكار مكتملة</span><span className="lifeos-chip">{lessons.length} درس محفوظ</span></div>} />

      <div className="lifeos-v3-summary-grid">
        <SummaryCard value={todayPages} label="صفحات اليوم" />
        <SummaryCard value={totalPagesRead} label="إجمالي القراءة" />
        <SummaryCard value={`${averageMemorization}%`} label="متوسط الحفظ" />
        <SummaryCard value={lessons.length} label="الدروس" />
      </div>

      {/* Tabs */}
      <div className="lifeos-v3-spiritual-tabs">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} data-active={tab === t.id} className="lifeos-v3-spiritual-tab">
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'dhikr' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => { setEditDhikr(null); setDhikrForm({ name: '', targetCount: 33 }); setShowDhikrForm(true); }} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-xl text-sm font-medium hover:bg-primary/90">
              <Plus size={16} /> إضافة ذكر
            </button>
          </div>
          {showDhikrForm && (
            <FormModal open={showDhikrForm} title={editDhikr ? 'تعديل الذكر' : 'إضافة ذكر'} onClose={() => { setShowDhikrForm(false); setEditDhikr(null); }}>
            <div className="bg-card rounded-2xl border border-primary/30 p-5 space-y-3">
              ...
              <div className="flex gap-3 justify-end">
                <button onClick={() => { setShowDhikrForm(false); setEditDhikr(null); }} className="px-4 py-2 rounded-xl text-sm border border-border text-muted-foreground">إلغاء</button>
                <button onClick={addDhikr} className="px-4 py-2 rounded-xl text-sm bg-primary text-primary-foreground">{editDhikr ? 'حفظ' : 'إضافة'}</button>
              </div>
            </div>
            </FormModal>
          )}
                ...
                <div key={dhikr.id} className={`bg-card rounded-2xl border p-5 ${completed ? 'border-primary/30' : 'border-border'}`}>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => { setEditDhikr(dhikr); setDhikrForm({ name: dhikr.name || '', targetCount: Number(dhikr.targetCount || 33) }); setShowDhikrForm(true); }} className="p-2 hover:bg-muted rounded-lg text-muted-foreground hover:text-foreground" aria-label="تعديل الذكر"><Edit3 size={13} /></button>
                      <button onClick={() => deleteDhikr(dhikr.id)} className="p-2 hover:bg-destructive/10 rounded-lg text-muted-foreground hover:text-destructive" aria-label="حذف الذكر"><Trash2 size={13} /></button>
                    </div>
                  </div>
          ...
          {dhikrList.length === 0 && (
            <AnimatedEmptyState icon={BookOpen} title="ابدأ قائمة أذكارك" description="أضف ذكرًا وحدد العدد المستهدف ليظهر تقدمك اليومي بوضوح." action={<button onClick={() => { setEditDhikr(null); setDhikrForm({ name: '', targetCount: 33 }); setShowDhikrForm(true); }} className="lifeos-primary-action"><Plus size={15} /> إضافة ذكر</button>} />
          )}
        </div>
      )}
      ...
      {tab === 'quran' && (
        <div className="space-y-4">
          ...
          {showQuranForm && (
            <FormModal open={showQuranForm} title={'تسجيل جلسة تلاوة'} onClose={() => setShowQuranForm(false)}>
          ...
          <div className="bg-card rounded-2xl border border-border p-5">
            ...
            {(quranData?.sessions || []).length === 0 ? (
              <AnimatedEmptyState icon={BookOpen} title="لا توجد جلسات قراءة بعد" description="سجّل أول جلسة لتبدأ متابعة الصفحات والورد اليومي." />
            ) : (
      ...
      {tab === 'memorization' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => { setEditMem(null); setMemForm({ juz: 1, surah: '', percentage: 0, lastReview: localDateKey() }); setShowMemForm(true); }} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-xl text-sm font-medium hover:bg-primary/90">
              <Plus size={16} /> إضافة
            </button>
          </div>

          {showMemForm && (
            <FormModal open={showMemForm} title={editMem ? 'تعديل سجل الحفظ' : 'إضافة سجل حفظ'} onClose={() => { setShowMemForm(false); setEditMem(null); }}>
            <div className="bg-card rounded-2xl border border-primary/30 p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium mb-1.5">السورة</label>
              ...
              <div className="flex gap-3 justify-end">
                <button onClick={() => { setShowMemForm(false); setEditMem(null); }} className="px-4 py-2 rounded-xl text-sm border border-border text-muted-foreground">إلغاء</button>
          ...
          <div className="space-y-3">
              ...
              <div key={m.id} className="bg-card rounded-xl border border-border p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => { setEditMem(m); setMemForm({ juz: Number(m.juz || 1), surah: m.surah || '', percentage: Number(m.percentage || 0), lastReview: m.lastReview || localDateKey() }); setShowMemForm(true); }} className="p-2 hover:bg-muted rounded text-muted-foreground hover:text-foreground" aria-label="تعديل سجل الحفظ"><Edit3 size={13} /></button>
                    <button onClick={() => { api(`/memorization/${m.id}`, { method: 'DELETE' }); setMemorization(x => x.filter(i => i.id !== m.id)); }} className="p-2 hover:bg-destructive/10 rounded text-muted-foreground hover:text-destructive" aria-label="حذف سجل الحفظ">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
            ...
            {memorization.length === 0 && (
              <AnimatedEmptyState icon={BookOpen} title="ابدأ سجل الحفظ" description="أضف سورة ونسبة الحفظ وتاريخ آخر مراجعة لتظهر لك صورة واضحة عن التقدم." />
            )}
          </div>
        </div>
      )}
      ...
      {tab === 'lessons' && (
        <div className="space-y-4">
          <div className="flex gap-3 flex-wrap">
            ...
            <button onClick={() => { setEditLesson(null); setLessonForm({ title: '', scholar: '', url: '', duration: '', notes: '' }); setShowLessonForm(true); }} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-xl text-sm font-medium hover:bg-primary/90">
              <Plus size={16} /> درس جديد
            </button>
          </div>

          {showLessonForm && (
            <FormModal open={showLessonForm} title={editLesson ? 'تعديل الدرس الديني' : 'إضافة درس ديني'} onClose={() => { setShowLessonForm(false); setEditLesson(null); }}>
            <div className="bg-card rounded-2xl border border-primary/30 p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div className="flex gap-3 justify-end">
                <button onClick={() => { setShowLessonForm(false); setEditLesson(null); }} className="px-4 py-2 rounded-xl text-sm border border-border text-muted-foreground">إلغاء</button>
              ...
              <div key={lesson.id} className="bg-card rounded-xl border border-border p-4">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1 mr-2">
                    <button onClick={() => { setEditLesson(lesson); setLessonForm({ title: lesson.title || '', scholar: lesson.scholar || '', url: lesson.url || '', duration: lesson.duration || '', notes: lesson.notes || '' }); setShowLessonForm(true); }} className="p-2 hover:bg-muted rounded-lg text-muted-foreground hover:text-foreground" aria-label="تعديل الدرس"><Edit3 size={13} /></button>
                    <button onClick={() => deleteLesson(lesson.id)} className="p-2 hover:bg-destructive/10 rounded-lg text-muted-foreground hover:text-destructive" aria-label="حذف الدرس"><Trash2 size={13} /></button>
                  </div>
                </div>
              </div>
            ))}
            {filteredLessons.length === 0 && (
              <AnimatedEmptyState icon={BookOpen} title="مكتبة الدروس فارغة" description="احفظ درسًا أو رابطًا مهمًا ليبقى ضمن مكتبتك الدينية للرجوع إليه لاحقًا." />
            )}
          </div>
        </div>
      )}
