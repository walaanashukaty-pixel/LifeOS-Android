...
function isOpenAIResponsesEndpoint(endpoint: string) {
  try {
    ...
    return url.hostname === 'api.openai.com' && /\/v1\/responses\/?$/.test(url.pathname);
  } catch {
    return false;
  }
}

function providerRequestBody({ endpoint, model, mode, message, context, conversationHistory }: {
  endpoint: string;
  model: string;
  ...
  const system = `${LIFEOS_AGENT_SYSTEM_PROMPT}\n\n${modeInstruction(mode)}`;
  const userPayload = JSON.stringify({ mode, message, context, conversationHistory });
...
export async function callLifeOSProvider({ endpoint, apiKey, model, message, context, history = [], mode = 'chat', fetchImpl = fetch }: {
  ...
  const response = await fetchImpl(endpoint, {
    ...
    body: JSON.stringify(providerRequestBody({ endpoint, model, mode, message, context, conversationHistory })),
  });
  if (!response.ok) {
    let detail = '';
    try {
      const errorPayload = await response.json();
      detail = typeof errorPayload?.error?.message === 'string' ? errorPayload.error.message : '';
    } catch {}
