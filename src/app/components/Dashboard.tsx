...
import { occursOnDate, previousOccurrenceDates } from '../../utils/recurrence';
...
import { queueAgentLaunch } from '../agent/agent-launch.ts';
import { buildPhase2Launch } from '../agent/phase2-modes.ts';
import {
...
export function Dashboard({ setPage }: DashboardProps) {
  const { user } = useAuth();
  const { proExperienceEnabled } = useMonetization();
  ...
  const todayHabits = habits.filter(h => habitOccursOnDate(h, today));
  ...
  const proInsight = buildProDashboardInsight({ date: today, tasks, events });

  function showProDayPlan() {
    queueAgentLaunch(buildPhase2Launch('day_plan'));
    setPage('ai');
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
                  ...
                  <div key={t.id} className={`flex items-center gap-3 py-2 px-3 rounded-xl transition-colors ${isCompleted ? 'bg-emerald-500/5' : isMissed ? 'bg-red-500/5' : 'hover:bg-muted/50'}`}>
                    ...
                    {isCompleted ? (
                      ...
                      <span className="text-[10px] text-muted-foreground flex-shrink-0">
                        {new Date(t.endDate + 'T12:00:00').toLocaleDateString('ar-SA', { month: 'short', day: 'numeric' })}
                      </span>
                    ) : null}
                  </div>
                );
      ...
      <div className="bg-card rounded-2xl border border-border p-5">
        ...
        {todayHabits.length === 0 ? (
          <EmptyState label={habits.length ? 'لا توجد عادات مطلوبة اليوم' : 'لا توجد عادات بعد'} action={habits.length ? 'عرض العادات' : 'أضف عادة'} onClick={() => setPage('habits')} />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
            {todayHabits.slice(0, 6).map((h: any, i) => {
              ...
              return (
                <div key={h.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${done ? 'border-primary/20 bg-primary/5' : 'border-border'}`}>
                  ...
                  <span className="text-xs text-muted-foreground flex-shrink-0">{calcStreak(h)}🔥</span>
                </div>
              );
            })}
          </div>
...
function habitOccursOnDate(habit: any, date: string): boolean {
  return occursOnDate({
    anchorDate: habit?.startDate || habit?.createdAt?.slice?.(0, 10) || date,
    targetDate: date,
...
function calcStreak(habit: any) {
  const today = localDateKey();
  const scheduled = previousOccurrenceDates({
  ...
  for (let i = 0; i < scheduled.length; i += 1) {
    ...
    const log = habit?.logs?.find?.((item: any) => item.date === date);
    if (i === 0 && date === today && !log) continue;
